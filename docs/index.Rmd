...
packages <- c("shiny", "rlang", "caret", "dplyr", "ggplot2", "readr", "rmarkdown", "htmltools")
...
library(ggplot2)
library(readr)
library(rmarkdown)  # Ensure this loads correctly
library(knitr)
library(htmltools)
...
cat("Value at time 6 from Present value =", pv * discount(6, 0))

cat("Difference between Accumulated Value and Present Value =", av - pv)
```
...
cat("Yearly deposits", yearly_deposit)
...
cat("Present value of the repayments", PV_repayments)
```
...
### Probability an invidual of age X dies before age Y, future life of age X

#### Finding the probability that an 23 year old dies before turning 24
```{r}

cat("qx value", qx[23 + 1])
